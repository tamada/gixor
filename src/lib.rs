...
impl Display for GibbsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GibbsError::Array(errs) => {
            ...
            GibbsError::NotFound(name) => write!(f, "{}: not found", name),
            GibbsError::Git(e) => write!(f, "Git error: {}", e),
            GibbsError::IO(e) => write!(f, "IO error: {}", e),
            GibbsError::Json(e) => write!(f, "JSON error: {}", e),
            GibbsError::Fatal(msg) => write!(f, "Fatal error: {}", msg),
        }
    }
}
...
impl<'a> Boilerplate<'a> {
    pub fn new(name: String, path: PathBuf, repo: &'a Repository) -> Boilerplate<'a> {
        Boilerplate { name, path, repo }
    }

    pub fn dump<P: AsRef<Path>>(&self, base_path: P) -> Result<String> {
        let bpath = base_path.as_ref();
        let hash = self.repo.hash(bpath)?;
        ...
        let hash_string = hash
            ...
            .collect::<String>();
        Ok(format!(
            r#"### Generated by Gibbs (https://github.com/tamada/gibbs)
...
"#,
            ...
            self.repo.repo_name,
            hash_string,
            to_relative_path(&self.path, bpath.join(&self.repo.name)),
            content
        ))
    }
}
    ...
    fn default() -> Self {
        match dirs::config_dir() {
            Some(dir) => {
                ...
                let config = Config {
                    repositories,
                    base_path: dir.join("gibbs").join("boilerplates"),
                };
                Self {
                    config,
                    load_from: dir.join("gibbs").join("config.json"),
                }
            }
...
impl Gibbs {
    ...
    /// If not exists, return the default configuration by [`Gibbs::default`].
    /// The default location is as follows.
    /// - Linux: `$XDG_CONFIG_HOME/gibbs/config.json` or `$HOME/.config/gibbs/config.json`
    /// - macOS: `$HOME/Library/Application Support/gibbs/config.json`
    /// - Windows: `{FOLDERID_RoamingAppData}\gibbs\config.json`
    pub fn load_or_default() -> Self {
        match dirs::config_dir() {
            Some(dir) => {
                let path = dir.join("gibbs").join("config.json");
                match Gibbs::load(path) {
                    Ok(s) => return s,
                    Err(_) => Gibbs::default(),
                }
            }
    ...
    pub fn repository<N: AsRef<str>>(&self, name: N) -> Option<&Repository> {
        ...
        self.config.repositories.iter().find(|repo| repo.name == name)
    }
    ...
    pub fn dump<N: AsRef<str>>(&self, name: N) -> Option<Boilerplate> {
        ...
        if let Some((path, repo)) = self.config.find(name.clone()) {
            Some(Boilerplate::new(name, path, repo))
        } else {
            None
        }
    }
    ...
    pub fn add_repository(&self, repo: Repository) -> Result<Gibbs> {
    ...
    pub fn add_repository_of<S: AsRef<str>>(&self, url: S) -> Result<Gibbs> {
        let repo = Repository::new(url);
        self.add_repository(repo)
    }

    pub fn remove_repository_with<S: AsRef<str>>(&self, name: S, keep_repo_dir: bool) -> Result<Gibbs> {
        let name = name.as_ref();
        let mut repos = self.config.repositories.clone();
        ...
        if let Some(index) = index {
            ...
            Err(GibbsError::Fatal(format!("{}: repository not found", name)))
        }
    }
    ...
    pub fn remove_repository<S: AsRef<str>>(&self, name: S) -> Result<Gibbs> {
        ...
        if path.exists() {
            if dot_git.exists() {
                match git::pull(&path, "origin", "main") {
                    Ok(_) => Ok(()),
                    Err(e) => Err(GibbsError::Git(e)),
                }
            } else {
                Err(GibbsError::Fatal(format!("{}: The repository is not a git repository", path.display())))
            }
        } else {
    ...
    fn clone_repository(&self, repo: &Repository) -> Result<()> {
        ...
        log::info!("Cloning the repository: {} into {}", repo.url, path.display());
        match git2::Repository::clone(&repo.url, &path) {
            Err(e) => Err(crate::GibbsError::Git(e)),
            Ok(_) => Ok(()),
        }
    }
}
...
impl Repository {
    ...
    pub fn hash<P: AsRef<Path>>(&self, base_path: P) -> Result<Vec<u8>> {
        ...
        let gitrepo = match git2::Repository::open(path) {
            Ok(repo) => repo,
            Err(_) => {
                return Err(GibbsError::Git(git2::Error::from_str(
                    format!("{}: Failed to open the repository", self.path.to_string_lossy().to_string()).as_str(),
                )))
            }
        };
        ...
        match head {
            ...
            Err(_) => 
                Err(GibbsError::Git(git2::Error::from_str(
...
mod tests {
    ...
    #[test]
    fn parse_gibbs() {
        match Gibbs::load(PathBuf::from("testdata/config.json")) {
            Err(e) => panic!("Failed to parse the config file: {}", e),
            Ok(gibbs) => {
                assert_eq!(
                    gibbs.config.base_path,
                    PathBuf::from("testdata/boilerplates")
                );
                assert_eq!(gibbs.config.repositories.len(), 1);
            }
        }
    }
    ...
    #[test]
    fn test_repo() {
        let repo = Repository::new_with_path("tamada", "git@github.com:tamada/gitignore.git", "tamada");
