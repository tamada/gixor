...
impl Name {
    ...
    pub fn matches(&self, boilerplate: &Boilerplate) -> bool {
        ...
        && self.repository_name.as_ref().map(|s| s.to_lowercase() == boilerplate.repo.name.to_lowercase())
            .unwrap_or(true)
    }
}
...
impl Gixor {
    ...
    pub fn iter<'a>(&'a self) -> impl Iterator<Item = Boilerplate<'a>> {
        self.config.iter()
    }

    /// Find the boilerplate by the name.
    pub fn find(&self, name: Name) -> Option<Boilerplate> {
    ...
    pub fn add_repository(&self, mut repo: Repository) -> Result<Gixor> {
        match self.clone_repository(&mut repo) {
            Err(e) => Err(e),
            Ok(_) => {
...
impl Config {
    fn find<'a>(&'a self, name: Name) -> Option<Boilerplate<'a>> {
        for repo in &self.repositories {
    ...
    fn iter<'a>(&'a self) -> impl Iterator<Item = Boilerplate<'a>> {
        self.repositories
            .iter()
...
impl Repository {
    ...
    pub fn find<P: AsRef<Path>>(&self, name: &Name, base_path: P) -> Option<Boilerplate> {
        for boilerplate in self.iter(base_path) {
            if name.matches(&boilerplate) {
    ...
    pub fn iter<'a, P: AsRef<Path>>(&'a self, base_path: P) -> impl Iterator<Item = Boilerplate<'a>> {
        let base_path = base_path.as_ref().to_path_buf();
        ignore::WalkBuilder::new(base_path.join(&self.path))
            .standard_filters(true)
            .build()
            .flatten()
            .map(|entry| entry.into_path())
            .filter(|p| is_gitignore_file(p.file_name()))
            .map(move |path| Boilerplate::new(
    ...
    pub fn iter2<P: AsRef<Path>>(&self, base_path: P) -> impl Iterator<Item = PathBuf> {
...
mod tests {
    ...
    fn test_repo() {
        ...
        if let Some(b) = repo.find(&Name::new_of("devcontainer"), &base_path) {
            assert_eq!(b.path, PathBuf::from("testdata/boilerplates/tamada/devcontainer.gitignore"));
        } else {
    ...
    fn test_target_name() {
        ...
        assert_eq!(target.repository_name, Some("default".to_string()));
        assert_eq!(target.boilerplate_name, "devcontainer");
    }
}
